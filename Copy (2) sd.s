;Todo:
;- make everything big endian (if performance increase!)

.include "inc/macro.s"

.export sd_current_cluster          ;starting cluster of sequence
.export sd_sequense_type            ;0=files list, 1=read file
.export sd_eos                      ;1=end of sequence


;Note:
; - In rare occasions read_first and read_next kan result in nothing: 255 lfn divided over 2 or 3 clusters


;File list def:
;$00 - Length of file or enf-of-list-marker ($00)
;$xx-$xx filename
;$xx-$xx+4 -> file size
;$xx-$xx+4 -> starting cluster
...
start:
    ...
    stz files_cnt
    stz sd_sequense_type    ;0=file list, so parse it
    jsr read_first
    

    :
        jsr print_list
        lda sd_eos  ;is end of sequence?
        bne at_end_of_sequence3
...
entry_is_lfn:
    ...
    ;read filename chars from 3 position within the file list entry. Unroled for better performance
...
read_next:
    jsr read_sequence
    bcs at_end_of_sequence
    lda sd_sequense_type
    beq parse_files_list
    ;use 
    rts
at_end_of_sequence:
    lda #1
...
read_sequence:
    ;start read marker, if still $FF when finished: just read again

    lda fat32_SecPerClus
    jsr dbg
...
goto_next_line_with_reset_lfn: jmp next_line_with_reset_lfn
goto_is_lfn:    jmp is_lfn
goto_normal_filename:   jmp normal_filename
;Parse sector data into files list



sector_to_files_list:
    lda #<sd_buffer
    sta r0
    lda #>sd_buffer
    sta r0+1
    ...
    beq at_end_of_list
    cmp #$E5    ;deleted
    beq goto_next_line_with_reset_lfn

    ldy #$0B    ;attributes
    lda (r0),y
    cmp #$0f
    beq goto_is_lfn
        ;is short filename, can be the shortname entry of an lfn
        sta (r1)    ;store attribyte byte
        lda sd_is_lfn
        beq goto_normal_filename
            ;print full filenamebuffer to screen
            jsr dbg_lf
            lda sd_lfn_buffer_index
            clc
            adc #13
            phy
                ...
                sta r1+1
            ply
            ;reset lfn buffer index
            lda #(255-13)
            sta sd_lfn_buffer_index
        jmp next_line_with_reset_lfn
        normal_filename:
        
        lda (r0),y
        AND #%00000010  ;check if file is hidden
        bne goto_next_line_with_reset_lfn2
        ;print 11 filename chars of short filename to screen
        jsr dbg_lf
        ldy #0
        :
            lda (r0),y
            ...
            bne :-
    jmp line_done
 goto_next_line_with_reset_lfn2: jmp next_line_with_reset_lfn   
    is_lfn:
    ...
    adc #0
    sta r0+1
    cmp #(>sd_buffer+$02)   ;are we past the end of the 512byte buffer?
    bne goto_next_line
        

    rts
goto_next_line: jmp next_line

read_sector:
   ; jmp read_sector_internal
